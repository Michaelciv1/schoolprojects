...
void searchArray(ofstream &output, string dictionary[], string keyword[], int DNUMBER, int KNUMBER); //used actual array names to avoid confusing myself for this specific method
void swapValues(string array[], int idx1, int idx2);
int smallestValueIndex(const string array[], int length, int startIndex = 0);
...
int main() {
    ...
    selectionSort(dictionary, DNUMBER);
    selectionSort(keyword, KNUMBER);
...
int smallestValueIndex(const string array[], int length, int startIndex){
    int minIdx = startIndex;

    for (int idx = startIndex+1; idx < length; idx++){
...
void selectionSort(string array[], int length){
    ...
    for (int idx = 0; idx < length; idx++){
        minIdx = smallestValueIndex(array, length, idx);
        if (minIdx != idx);
            swapValues(array, minIdx, idx);
    }
}
